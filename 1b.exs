...
match = fn(line) ->
  submatch = fn
    "", acc, _ -> acc
    substr, acc, recur ->
      next_substr = String.slice(substr, 1..-1//1)
      acc = acc ++ (Regex.run(regex, substr, capture: :first) |> List.wrap())
      recur.(next_substr, acc, recur)
  end
  submatch.(line, [], submatch)
end

sum = Enum.reduce(lines, 0, fn(line, acc) ->
  matches = match.(line)
  first = List.first(matches)
  last = List.last(matches)
  num_str = Map.get(digits, first, first) <> Map.get(digits, last, last)
