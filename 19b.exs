...
[workflow_lines, _] = String.split(input, "\n\n") |> Enum.map(&String.split(&1))

# %{name => [{:param, :operator, :operand, :next}]}
workflows =
  workflow_lines
  |> Enum.map(fn line ->
    [_, name, steps_str, last_next] = Regex.run(~r/(\w+)\{(?:(.+),)?(\w+)\}/, line)

    steps =
      steps_str
      |> String.split(",")
      |> Enum.map(fn step_str ->
        [_, param, operator, operand, next] = Regex.run(~r/(\w+)([<>])(\d+):(\w+)/, step_str)
        {param, operator, String.to_integer(operand), next}
      end)

    {name, steps ++ [{"z", "", "", last_next}]}
  end)
  |> Enum.into(%{})

defmodule DAGCrawler do
  # steps = [{:param, :operator, :operand, :next}, ...]
  # paths = [[{:param, :operator, :operand, :next}, ...], ...]
  def crawl(_workflows, steps, paths) when length(steps) == 0 do
    paths
  end

  def crawl(workflows, steps, paths) do
    for {curr_step, curr_step_n} <- Enum.with_index(steps) do
      negated_steps =
        for {param, operator, operand, next} <- Enum.slice(steps, 0, curr_step_n) do
          case operator do
            "<" -> {param, ">", operand - 1, next}
            ">" -> {param, "<", operand + 1, next}
            "" -> dbg([curr_step, curr_step_n])
          end
        end

      curr_steps = negated_steps ++ [curr_step]

      {_param, _operator, _operand, next} = curr_step
      next_paths = paths ++ curr_steps
      ...
      case next do
        ...
        _ ->
          next_steps = workflows[next]
          crawl(workflows, next_steps, next_paths)
      end
    end
    |> Enum.flat_map(& &1)
  end
...
routes = DAGCrawler.crawl(workflows, workflows["in"], [])
# dbg(routes)
...
ans =
  for route <- routes do
    ranges =
      for step <- route, reduce: init_ranges do
        acc ->
          {param, operator, operand, _} = step

          case operator do
            "<" ->
              if operand < acc[param][:max],
                do: Map.put(acc, param, Map.put(acc[param], :max, operand - 1)),
                else: acc

            ">" ->
              if operand > acc[param][:min],
                do: Map.put(acc, param, Map.put(acc[param], :min, operand + 1)),
                else: acc
    ...
    for {_, %{min: rmin, max: rmax}} <- ranges do
      rmax - rmin + 1
    end
    |> Enum.product()
  end
...
# thought it was a tree structure at first and tried to inverse the tree such that we can get "A" => list of paths to "in", but realized it's a DAG instead

# # %{name => [{prev workflow name, index of step in prev workflow}]}
# inverse_steps =
#   for {name, steps} <- workflows, reduce: %{} do
#     acc ->
#       for {{_, _, _, next_name}, i} <- Enum.with_index(steps), reduce: acc do
#         acc2 ->
#           new_prev_names = Map.get(acc2, next_name, []) ++ [{name, i}]
#           Map.put(acc2, next_name, new_prev_names)
#       end
#   end

# dbg(inverse_steps)
# a_prev_steps = inverse_steps["A"]

# a_inverser =
#   for prev_step <- a_prev_steps do
#     Enum.reduce_while(0..length(workflow_lines), {prev_step, []}, fn _, {curr_step, acc_steps} ->
#       {curr_name, step_n} = curr_step

#       if curr_name == "in" do
#         {:halt, acc_steps}
#       else
#         steps = workflows[curr_name]

#         negated_steps =
#           for {param, operator, operand, _} <- Enum.slice(steps, 0, step_n) do
#             case operator do
#               "<" -> {param, ">", operand - 1}
#               ">" -> {param, "<", operand + 1}
#               "" -> dbg([curr_name, step_n])
#             end
#           end

#         {last_param, last_operator, last_operand, _} = Enum.at(steps, step_n)
#         curr_steps = negated_steps ++ [{last_param, last_operator, last_operand}]

#         {:cont, {prev_name, curr_steps ++ acc_steps}}
#       end
#     end)
#   end
...
# dbg(a_inverser)
# in{s<1351:px,qqz}
# %{"px" => [{s<1351}], "qqz" => [{s>=1351}]}
# px{a<2006:qkq,m>2090:A,rfg}
# %{"px" => [{s<1351}], "qqz" => [{s>=1351}], "qkq" => [{s<1351}, {a<2006}], "A" => [{s<1351}, {a>=2006}, {m>2090}], "rfg" => [{s<1351}, {a>=2006}, {m<=2090}]}
