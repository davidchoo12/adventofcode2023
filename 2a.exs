...
game_sum = Enum.map(lines, fn(line) ->
  [_, game, sets] = Regex.run(~r/Game (\d+): (.+)/, line)
  count_colors = Regex.scan(~r/(\d+) (\w+)/, sets)
  impossible = for [_, count, color] <- count_colors, String.to_integer(count) > color_maxcount[color] do
    true
  end |> Enum.any?()
  unless impossible, do: String.to_integer(game), else: 0
end) |> Enum.sum()
