...
matrix = Enum.map(lines, & String.split(&1, "", trim: true))
# IO.inspect(matrix)

symbols_xy = for {row, i} <- Enum.with_index(matrix),
{char, j} <- Enum.with_index(row),
Integer.parse(char) == :error && char != "." do
  {i, j}
end
# IO.inspect(symbols_xy)

ints = for {line, i} <- Enum.with_index(lines), [match] <- Regex.scan(~r/\d+/, line, return: :index) do
  {y, len} = match
  %{
    x: i,
    y: y,
    len: len,
    val: String.to_integer(String.slice(line, y, len)),
  }
end
# IO.inspect(ints)

sum = Enum.map(symbols_xy, fn {sx, sy} ->
  adj_ints = Enum.filter(ints, fn %{x: x, y: y, len: len} -> x-1 <= sx and sx <= x+1 and y-1 <= sy and sy <= y+len end)
  if length(adj_ints) == 2, do: Enum.map(adj_ints, & &1.val) |> Enum.product(), else: 0
end) |> Enum.sum()
