...
[workflow_lines, parts_lines] = String.split(input, "\n\n") |> Enum.map(&String.split(&1))
...
parts =
  ...
  |> Enum.map(fn line ->
    ...
    |> String.split(",")
    |> Enum.map(fn param_str ->
      [param, n] = String.split(param_str, "=")
      {param, String.to_integer(n)}
    end)
    |> Enum.into(%{})
  end)
...
ans =
  Enum.filter(parts, fn part ->
    Enum.reduce_while(0..length(workflow_lines), "in", fn _, name ->
      if name == "A" || name == "R" do
        {:halt, name}
      else
        steps = Map.get(workflows, name)
        last_next = Enum.at(steps, -1) |> elem(3)

        next =
          Enum.reduce_while(steps, last_next, fn step, ret ->
            {param, operator, operand, next} = step
            n = part[param]

            case operator do
              "<" ->
                if n < operand, do: {:halt, next}, else: {:cont, ret}
