...
game_sum = Enum.map(lines, fn(line) ->
  [_, sets] = Regex.run(~r/Game \d+: (.+)/, line)
  count_colors = Regex.scan(~r/(\d+) (\w+)/, sets)
  for [_, count, color] <- count_colors do
    %{color: color, count: String.to_integer(count)}
end |> Enum.group_by(& &1.color) |> Enum.map(fn {_, rows} -> Enum.max_by(rows, & &1.count).count end) |> Enum.product()
end) |> Enum.sum()
